...
public class DBManager {
	...
	public static int creatTables()
	{
		...
		try {
			//Cr�ation de la table "ATHLETE"
			myRequest = "CREATE TABLE IF NOT EXISTS ATHLETE ("+
			"ID INTEGER PRIMARY KEY,"+ // Identifiant athl�te (11-infini)
...
System.out.println("INFO -- Create table : "+myRequest);
		...
		try {
			//Cr�ation de la table "ENTRAINEUR"
...
System.out.println("INFO -- Create table : "+myRequest);
		...
		try {
			...
			"ID INTEGER,"+ // 1<ID<=10 (Entraineur) /   ID>10 (Athl�te)
			"MOIS DATE,"+
			"MONTANT DECIMAL(8,2),"+ //8 chifres dont 2 apr�s la virgule
...
System.out.println("INFO -- Create table : "+myRequest);
		...
		try {
			//Cr�ation de la table "ASSURANCE"
			myRequest = "CREATE TABLE IF NOT EXISTS ASSURANCE ("+
			"ID INTEGER PRIMARY KEY,"+ // 1<ID<=10 (Entraineur) /   ID>10 (Athl�te)
			"debut DATE,"+
...
System.out.println("setEntraineur3 : "+myRequest);
	...
	public static ArrayList<ArrayList<Object>> getATHLETE(Integer id, String nom,String prenom) {
		...
		if (con == null) {
			...
			ret.add(0, "Probl�me de connection � la base de donn�es");
			athletes.add(0, ret);
		} else {
			try {
				String myRequest = "";
				if ((id != null && nom != null && prenom != null
					...
					&& id == 0 && nom.equals("Tapez le nom de l_athl�te") && prenom.equals("Tapez le pr�nom de l_athl�te"))){
					...
					if (nom!=null && !nom.isEmpty() && !nom.equals("Tapez le nom de l_athl�te")) 
						myRequest = myRequest + " OR NOM='" + nom + "'";
					if (prenom!=null && !prenom.isEmpty() && !prenom.equals("Tapez le pr�nom de l_athl�te")) 
...
System.out.println("set_PAIEMENT : "+myRequest);
	...
	public static ArrayList<Object> get_ASSURANCE(Integer id) {
		...
		Connection con = getConnection();
		if (con == null) {
			assurance.add(0, "Probl�me de connection � la base de donn�es");
		} else {
			try {
				String myRequest = "";
				if ((id != null	&& id == 0) ||
		...
		if (con == null) {
			...
			try {
				...
				if ((id != null && nom != null && prenom != null
					&& id == 0 && nom.isEmpty() && prenom.isEmpty()) ||
					(id != null && nom != null && prenom != null
					&& id == 0 && nom.equals("Tapez le nom de l_entraineur") && prenom.equals("Tapez le pr�nom de l_entraineur"))){
					...
					if (nom!=null && !nom.isEmpty() && !nom.equals("Tapez le nom de l_entraineur")) 
						myRequest = myRequest + " OR NOM='" + nom + "'";
					if (prenom!=null && !prenom.isEmpty() && !prenom.equals("Tapez le pr�nom de l_entraineur")) 
						myRequest = myRequest + " OR PRENOM='" + prenom + "'";
				}
...
System.out.println("getATHLETE_non_assures : "+myRequest);
	...
	public static ArrayList<Object> getATHLETE_non_payes() {
		...
		Connection con = getConnection();
		if (con == null) {
			athletes.add("Probl�me de connection � la base de donn�es");
		} else {
			try {
				String myRequest = "";
				String d = date.format(formatter);
